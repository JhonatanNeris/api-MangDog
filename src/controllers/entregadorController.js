    ...
    static async getResumoEntregadores(req, res, next) {
        ...
        try {
            ...
            if ((dataInicio && !dataFim) || (!dataInicio && dataFim)) {
                return res.status(400).json({ erro: 'dataInicio e dataFim devem ser informados juntos (YYYY-MM-DD).' });
            }
            ...
            if (dataInicio && dataFim) {
                ...
                inicio = new Date(`${dataInicio}T${String(hiH).padStart(2, '0')}:${String(hiM).padStart(2, '0')}:00-03:00`);
                fim = new Date(`${dataFim}T${String(hfH).padStart(2, '0')}:${String(hfM).padStart(2, '0')}:59.999-03:00`);
            }
                        ...
                        pipeline: [
                            {
                                $match: {
                                    ...
                                    clienteId: req.usuario.clienteId,
                                    tipoPedido: 'delivery',
                                    ...(inicio && fim ? { createdAt: { $gte: inicio, $lte: fim } } : {})
                                }
                            },
                            {
                                $lookup: {
            ...
            res.json({ periodo: inicio && fim ? { inicio, fim } : null, filtros: { busca, entregadorId }, data });
        } catch (e) {
            console.error(e);
